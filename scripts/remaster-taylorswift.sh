...
function update_new_iso {
	...
	cp $DIR_CONFIG/new-iso/menu.lst $REM/new-iso/boot/grub/menu.lst

	rm $REM/new-iso/boot/grub/menu.lst.extra
	cp $DIR_CONFIG/new-iso/menu.lst.extra $REM/new-iso/boot/grub/menu.lst.extra

	rm $REM/new-iso/version
	cp $DIR_CONFIG/new-iso/version $REM/new-iso/version
...
function cleanup {
	...
	#Y_or_n && {
		echo -e "Removing 1/root/.synaptic/log/*"
		rm -f $1/root/.synaptic/log/*
		echo -e "Removing 1/root/.bash_history"
...
function chroot_env {
	...
	chroot $1 sh $DIR_SCRIPT/special/shared_taylorswift.sh # Transform antiX Linux into Regular Swift Linux in chroot

	umount_all $1
	cleanup $1

	# After chroot operations, remove the development files from the new-squashfs directory.
...
function set_iso_path {
	cd $STARTPATH
	echo -e "The ISO file will be placed in the \"$REM\" directory. \n"
	ISOPATH=$REM
	#echo -n "Is that OK"
	#Y_or_n && ISOPATH=$REM || {
	#	while true; do 
...
function make_iso {
	cd $STARTPATH
	mkisofs -l -r -R -v -V "antiX" -no-emul-boot -boot-load-size 5 -boot-info-table -b boot/grub/stage2_eltorito -c boot.catalog -hide-rr-moved -o $1 $REM/new-iso
	if [[ $? -eq 0 ]]; then 
		echo
